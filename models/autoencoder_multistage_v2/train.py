...
#autoencoder_filepath = "VIT-BCE-VGG-98-epoch042"
...
#optimizer_filepath = "Opt-BCE-VGG-98-epoch042"
...
lr_scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.75, patience=4, threshold=1, cooldown=2, min_lr=1e-7, eps=1e-08)
class EarlyStopping:
...
#epoch_loss_Pixel = np.load("epoch_loss_pixel-epoch42.npy")[:42]
#epoch_loss_VGG = np.load("epoch_loss_VGG-epoch42.npy")[:42]
#validation_epoch_loss = np.load("validation_epoch_loss-epoch42.npy")[:42]
...
for epoch in epoch_loop:
    ...
    for n, image_batch in enumerate(tqdm(train_dataloader,total=len(train_dataloader))):
        ...
        y_hat = autoencoder_model.decode(latent, logits=False)
